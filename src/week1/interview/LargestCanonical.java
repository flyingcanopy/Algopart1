 ...
 *
 ...
 We maintain that largest value of the connected component at the entry Largest[root(nodex)]
 We use an extra space of size N .

 For each successfull union(p,q) call
 {
 ->calculate maxl(largest[root(p)],largest[root(q)])
 ->find out what will be the rootx when two connected components are merged to give one connected component.
